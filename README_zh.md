...
### ⌨️ 模块�?PowerShell 配置，支�?tmux 会话管理、Starship 提示符和跨平台工�?
...
### 🚀 性能
- **懒加�?* �?tmux 模块仅在你首次输�?`sss` 时才加载
- **快速启�?* �?最小化足迹，毫秒级加载

### 🛠�?开发者工�?
- **Starship 提示�?* �?�?Shell 提示符，显示 git 上下文、Node 版本
- **Fnm 集成** �?cd 切换项目时自动切�?Node 版本
- **PSReadLine** �?基于历史的自动补全，更好的导航体�?

### 🔧 远程会话
- **Tmux 管理�?* �?交互式菜单管理远�?tmux 会话
- **快速连�?* �?`sss <host>` 立即附加/恢复/创建会话
...
| 工具 | 用�?| 安装指南 |
|------|------|----------|
| [Starship](https://starship.rs/) | 美观提示符，�?git 上下�?| [安装](https://starship.rs/guide/#🚀-installation) |
| [Fnm](https://github.com/Schniz/fnm) | 快�?Node 版本管理�?| [安装](https://github.com/Schniz/fnm#installation) |
...
也可以使用任意自定义根路径，之后�?`Config.ps1` 中修改即可�?

### 3️⃣ 安装配置文件

```powershell
# 步骤 A：查�?PowerShell 配置目录
$PROFILE

# 步骤 B：将所�?.ps1 文件复制到配置目�?
#         �?"D:\path\to\PowerShell" 替换为实际克隆路�?
Copy-Item -Path "D:\path\to\PowerShell\*.ps1" `
           -Destination (Split-Path $PROFILE -Parent) `
           -Force

# 步骤 C：重�?PowerShell 或执行：
reload
```
...
```powershell
$global:UserScoop_CONF.Colors.Cyan  # 主色�?
...
�?`$UserScoop_ROOT\quotes.txt` 放置语录文件，每条语录用 `%` 分隔�?
